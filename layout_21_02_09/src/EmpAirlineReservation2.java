...
import dbAll.EmpReservation2DAO;
import dbAll.EmpReservation2VO;

public class EmpAirlineReservation2 extends JPanel implements MouseListener, ItemListener{
	Font fnt = new Font("굴림체", Font.BOLD, 14);
	Font titleFnt = new Font("굴림체", Font.BOLD, 32);
	
	JLabel titleLbl1 = new JLabel("선택 항공 내역");
	String flightStr[] = {"출발지", "출발일", "출발시간", "도착시간", "항공편명", "좌석", "운임"};
	JTable table1;
	JScrollPane sp1;
	...
	public EmpAirlineReservation2() {
		...
		add(payBtn).setBounds(370, 610, 100, 30);
			payBtn.setFont(fnt);
			payBtn.setBackground(new Color(255,128,128));
...
//				System.out.println(allSelected);
	...
	public void setMileageUpdate(int mileageTotal, int mileageOld, int mileageNew, String passNo) {//결제창에서 결제버튼 누르면 마일리지 업데이트
		...
		EmpReservation2DAO dao = new EmpReservation2DAO();
			...
			if(btnStr.equals("결제하기")) {
				if(allSelected==0) {
					...
					if(paymentWindowState==0) {
						...
						System.out.println("제발요//"+oldFare);
						System.out.println("제발요11//"+rowCount);
						
						if(oldFare>newFare) {
							JOptionPane.showMessageDialog(this, "예약 변경이 완료되었습니다. \n 이용해주셔서 감사합니다.");
							this.setVisible(false);
							EmpMainFrame.reservation3.setVisible(true);
							EmpMainFrame.centerPane.add(EmpMainFrame.reservation3);
						}else {
	...
	///**************************************************************************************TABLE1
		public void table1Print() {
			String newFlight = EmpAirlineReservation1.newFlightNum;//t1,t3
			
			String resNo = EmpAirlineReservation.resNOtest;//t1,t2
//			String oldFlight = EmpAirlineReservation1.getFlight;//t3
			
			setTable1(newFlight, resNo);
			System.out.println(newFlight+"        "+resNo);
//			System.out.println(oldFlight);
		}
		public void setTable1(String newFlight1, String resNo1) {
			EmpReservation2DAO dao = new EmpReservation2DAO();
			List<EmpReservation2VO> lst = dao.getTable3(newFlight1, resNo1);
			
			setTable1List(lst);
		}
		public void setTable1List(List<EmpReservation2VO> lst) {
			model1.setRowCount(0);
			for(int i=0; i<lst.size(); i++) {
				EmpReservation2VO vo = lst.get(i);
				Object[] data = {vo.getDep(),  vo.getBrdDate(), vo.getDepTime(),
						vo.getDesTime(), vo.getFlightNo(), vo.getSeatNo(), vo.getFare2()};
	...
	///**************************************************************************************TABLE2
		...
		public void setTable2(String resNo1) {
			EmpReservation2DAO dao = new EmpReservation2DAO();
			List<EmpReservation2VO> lst = dao.getTable4(resNo1);
			
			setTable2List(lst);
		}
		public void setTable2List(List<EmpReservation2VO> lst) {
	...
	public void test3() {
		...
		String num1 = EmpAirlineReservation.fno;
		String num2 = EmpAirlineReservation1.newFlightNum;
	...
	public void test(String depfare, String fare) {
		
		EmpReservation2DAO dao = new EmpReservation2DAO();
		
		List<EmpReservation2VO> lst = dao.EmpReservation2Allselect(depfare, fare);
	...
	public void setNewTableList(List <EmpReservation2VO> lst) {
		model3.setRowCount(0);
		
		for(int i = 0 ; i<1; i++) {
		EmpReservation2VO vo1 =lst.get(0);
		EmpReservation2VO vo2 =lst.get(1);

		
//		Object[] data1 = {vo.getFlightNO(),vo.getFare(),vo.getFlightNO2(),vo.getChangeFare()};
		Object data = vo1.getFare();
		Object[] data1 = {data, vo2.getFare()};
//		Object[] data1 = {vo.getFare(), vo.getChangeFare()};
...
//		JButton mileageBtn = new JButton("사용하기");
		...
		public void getMileage(String passNo) {
			...
			int oldFare = Integer.parseInt((String)EmpMainFrame.reservation2.model3.getValueAt(0, 0));
			int newFare = Integer.parseInt((String)EmpMainFrame.reservation2.model3.getValueAt(0, 1));
			...
			EmpMainFrame.reservation2.setMileageUpdate(mileageTotal, mileageOld, mileageNew, passNo);
		...
		public void actionPerformed(ActionEvent ae) {
			...
			if(obj instanceof JComboBox) {
				...
				if(btnStr.equals("취소")) {
					...
					String newFlightFromModel1 = (String)EmpMainFrame.reservation2.model1.getValueAt(0, 5);
					String resno = EmpMainFrame.reservation.fno;
					EmpMainFrame.reservation2.setBookingUpdate(newFlightFromModel1, resno);
					
					String passNo = (String)EmpMainFrame.reservation2.model2.getValueAt(0, 2);
					System.out.println(passNo);
					getMileage(passNo);
					
					EmpMainFrame.reservation3.table1Print();
					EmpMainFrame.reservation3.table2Print();
					this.setVisible(false);
//					paymentState = 1; // 1이면 결제를 했다는 뜻이니까 cbc4로 화면 전환 가자!!
//					System.out.println(paymentState);
					EmpMainFrame.reservation2.setVisible(false);
					EmpMainFrame.reservation3.setVisible(true);
					EmpMainFrame.centerPane.add(EmpMainFrame.reservation3);
				}
			}
		...
		public void mouseClicked(MouseEvent me) {
			...
			if(obj instanceof JButton) {
				...
				if(btnName.equals("결제")) {
					JOptionPane.showMessageDialog(this, "결제가 완료되었습니다");
					String newFlightFromModel1 = (String)EmpMainFrame.reservation2.model1.getValueAt(0, 5);
					String resno = EmpMainFrame.reservation.resNOtest;
					EmpMainFrame.reservation2.setBookingUpdate(newFlightFromModel1, resno);
