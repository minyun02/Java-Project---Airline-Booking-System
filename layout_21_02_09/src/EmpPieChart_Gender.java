...
public class EmpPieChart_Gender extends JFrame {
	...
	class NorthPane extends JPanel {
		...
		NorthPane() {
			...
			for(Map.Entry<String, String> data2 : EmpAirlineSales.data.entrySet()) {
					//연령대+인원수 라벨(상단에 위치)
					if(data2.getKey().equals("F")) {
						gender = "여성";
					}else if(data2.getKey().equals("M")) {
						gender = "남성";
					}
					countLbl = new JLabel(gender+" ▶ " + data2.getValue()+"명");
